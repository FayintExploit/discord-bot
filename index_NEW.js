...
const handleCommand = async (cmd, args, reply, userId, member, guild, message=null) => {
  ...
  if (cmd === "panelsetup") {
    ...
    try {
      ...
      const emb = new EmbedBuilder()
        .setTitle("\uD83D\uDD37 "+script.name+" | Premium Panel")
        .setDescription(
          ...
          "\uD83D\uDD04 Click **Reset HWID** to reset your HWID\n"+
      ...
      const row1 = new ActionRowBuilder().addComponents(
        new ButtonBuilder().setCustomId("panel_redeem__"+scriptId).setLabel("Redeem Key").setStyle(ButtonStyle.Success).setEmoji("\uD83D\uDD11"),
        new ButtonBuilder().setCustomId("panel_getscript__"+scriptId).setLabel("Get Script").setStyle(ButtonStyle.Primary).setEmoji("\uD83D\uDCDC"),
      );
      const row2 = new ActionRowBuilder().addComponents(
        new ButtonBuilder().setCustomId("panel_getrole__"+scriptId).setLabel("Get Role").setStyle(ButtonStyle.Primary).setEmoji("\uD83D\uDC65"),
        new ButtonBuilder().setCustomId("panel_resethwid__"+scriptId).setLabel("Reset HWID").setStyle(ButtonStyle.Danger).setEmoji("\uD83D\uDD04"),
        new ButtonBuilder().setCustomId("panel_getstats__"+scriptId).setLabel("Get Stats").setStyle(ButtonStyle.Secondary).setEmoji("\uD83D\uDCCA"),
      );
  ...
  if (cmd === "setupdatechannel") {
    ...
    return reply(E.check+"\uD83D\uDCE3 Update announcements set to <#"+chanId+">!");
  }
  ...
  if (cmd === "sendupdate") {
    ...
    const typeEmoji  = { major:"\uD83D\uDD34", minor:"\uD83D\uDD35", hotfix:"\uD83D\uDFE0", patch:"\uD83D\uDFE2" };
    ...
    const emb = new EmbedBuilder()
      .setTitle(typeEmoji[type]+"\uD83D\uDCE3 "+script.name+" Update | "+version)
      .setColor(typeColors[type]||0x3498db)
      .addFields(
        { name:"\uD83D\uDCCB Changelog", value:changelog.map(c=>"\u2022 "+c).join("\n"), inline:false },
    ...
    // Log it
    await sendLog("\uD83D\uDCE3 **Update sent** | `"+scriptId+"` "+version+" by <@"+userId+"> | Notified **"+notified+"** subscribers");

    return reply(E.check+"\uD83D\uDCE3 Update **"+version+"** sent!\n\uD83D\uDD14 Notified **"+notified+"** subscriber(s).\n"+E.chart+" "+changelog.length+" change(s) logged.");
  }
  ...
  if (cmd === "subscribeupdate") {
    ...
    return reply(E.check+"\uD83D\uDD14 Subscribed to **"+db.premiumScripts[scriptId].name+"** updates! You'll get DMs when there's a new update.");
  }
  ...
  if (cmd === "unsubscribeupdate") {
    ...
    return reply(E.check+"\uD83D\uDD15 Unsubscribed from **"+db.premiumScripts[scriptId]?.name+"** updates.");
  }
  ...
  if (cmd === "updatehistory") {
    ...
    const emb = new EmbedBuilder()
      .setTitle("\uD83D\uDCDC\uD83D\uDCE3 Update History | "+script.name)
      .setColor(0x3498db)
      .setDescription(history.slice(-5).reverse().map((u,i)=>{
        const typeEmoji = { major:"\uD83D\uDD34", minor:"\uD83D\uDD35", hotfix:"\uD83D\uDFE0", patch:"\uD83D\uDFE2" };
        return (typeEmoji[u.type]||"\uD83D\uDD35")+" **"+u.version+"** — <t:"+Math.floor(u.time/1000)+":R>\n"+u.changelog.slice(0,3).map(c=>"\u2022 "+c).join("\n");
      }).join("\n\n"))
  ...
  if (cmd === "activitylog") {
    ...
    const emb = new EmbedBuilder()
      .setTitle("\uD83D\uDCCB"+E.spark+" Activity Log | Last "+limit)
      .setColor(0x2c3e50)
  ...
  if (cmd === "mylog") {
    ...
    const emb = new EmbedBuilder()
      .setTitle("\uD83D\uDCCB"+E.spark+" Your Activity Log")
      .setColor(0x3498db)
      .setDescription(logs.map((l,i)=>
  ...
  if (cmd === "clearlog") {
    ...
    return reply(E.check+"\uD83E\uDDF9 Activity log cleared!");
  }
  ...
  if (cmd === "backup") {
    ...
    try {
      ...
      await user.send({
        content: E.check+"\uD83D\uDCBE **Database Backup** | "+date+"\nSize: **"+size+" KB**",
        files:[{ attachment: buf, name:"backup_"+date+".json" }]
      });
      return reply(E.check+"\uD83D\uDCBE Backup sent to your DMs! ("+size+" KB)");
  ...
  if (cmd === "restore") {
    ...
    try {
      ...
      return reply(E.check+"\uD83D\uDCBE Database restored successfully!");
  ...
  if (cmd === "statusadd") {
    ...
    return reply(E.check+"\uD83D\uDD04 Status added: **["+( type||"PLAYING")+"] "+text+"** ("+db.statusCfg.statuses.length+" total)");
  }
  ...
  if (cmd === "statuslist") {
    ...
    const emb = new EmbedBuilder()
      .setTitle("\uD83D\uDD04"+E.spark+" Status Rotation List")
      .setColor(0x3498db)
      .setDescription(statuses.map((s,i)=>"`"+(i+1)+"` **["+s.type+"]** "+s.text).join("\n"))
      .addFields(
        { name:"\uD83D\uDFE2 Enabled",  value: db.statusCfg?.enabled ? "Yes" : "No",                     inline:true },
        { name:E.timer+" Interval", value: "Every **"+(db.statusCfg?.interval||5)+" min**",             inline:true },
        { name:"\uD83D\uDD22 Current",  value: "Status #"+((db.statusCfg?.current||0)+1),                  inline:true },
      );
    return reply({ embeds:[emb] });
  }
  ...
  if (cmd === "statusrotate") {
    ...
    saveDB();
    if (action==="on") { startStatusRotator(); return reply(E.check+"\uD83D\uDD04 Status rotation **ON**! Rotating every **"+interval+" min**."); }
    else {
      if (statusTimer) clearInterval(statusTimer);
      client.user.setActivity(null);
      return reply(E.stop+"\uD83D\uDD04 Status rotation **OFF**.");
    }
  }
  ...
  if (cmd === "statssetup") {
    ...
    if (channelCh) lines.push("\uD83D\uDCE2 Channels → <#"+channelCh+">");
    return reply(E.check+"\uD83D\uDCCA Server stats setup!\n"+lines.join("\n")+"\n\n"+E.timer+" Auto-updates every **5 min**.");
  }
...
client.on("interactionCreate", async (interaction) => {
  ...
  if (interaction.isButton()||interaction.isStringSelectMenu()) {
    ...
    if (interaction.customId?.startsWith("panel_resethwid__")) {
      ...
      return interaction.reply({ content:"\uD83D\uDD04"+E.check+" HWID **reset**! You can now use the script on a new device.", ephemeral:true });
    }
    ...
    if (interaction.customId?.startsWith("panel_getstats__")) {
      ...
      const emb=new EmbedBuilder()
        ...
        .addFields(
          ...
          { name:"\uD83D\uDD04 HWID Reset", value:hwidLeft>0?"Cooldown: **"+d+"d "+h+"h**":E.check+" Ready!", inline:true },
    ...
    // MY TICKETS BUTTON (from panel)
    if (interaction.customId==="ticket_mytickets") {
      const myTks=Object.entries(db.tickets||{}).filter(([,t])=>t.userId===userId);
      if (!myTks.length) return interaction.reply({ content:"\uD83D\uDCA4 No tickets yet! Click **Buy Script** to create one.", ephemeral:true });
      const statusEmoji={ pending:"\u23F3 Pending", approved:"\u2705 Approved", rejected:"\uD83D\uDEAB Rejected" };
      const emb=new EmbedBuilder()
        .setTitle("\uD83C\uDFAB My Tickets")
        .setColor(0x3498db)
        .setDescription(myTks.slice(-5).reverse().map(([id,t])=>{
          const script=db.premiumScripts?.[t.scriptId];
          return (statusEmoji[t.status]||"Unknown")+" | **"+id+"**\n"+
            "\uD83D\uDCDC "+(script?.name||t.scriptId)+" | \uD83D\uDCB5 "+t.payMethod+"\n"+
            "<t:"+Math.floor(t.time/1000)+":R>";
        }).join("\n\n"))
        .setFooter({ text:"Last 5 tickets" });
      return interaction.reply({ embeds:[emb], ephemeral:true });
    }

    // Panel: Open Ticket button — show embed script list + select menu
    if (interaction.customId==="open_ticket") {
      const scripts=Object.entries(db.premiumScripts||{});
      if (!scripts.length) return interaction.reply({ content:"\uD83D\uDCA4 No scripts available yet!", ephemeral:true });

      const existing=Object.values(db.tickets||{}).find(t=>t.userId===userId&&t.status==="pending"&&t.guildId===interaction.guild?.id);
      if (existing) return interaction.reply({ content:"\u26A0\uFE0F You already have an open ticket! Check <#"+existing.channel+">.", ephemeral:true });

      const emb=new EmbedBuilder()
        .setTitle("\uD83D\uDCDC Premium Scripts Available")
        .setColor(0x7c3aed)
        .setDescription(
          scripts.map(([id,s])=>
            "\uD83D\uDCDC **"+s.name+"**\n"+
            "\uD83E\uFA99 "+s.price+" | \uD83D\uDCB5 "+s.payment.join(", ")+"\n"+
            "ID: `"+id+"`"
          ).join("\n\n")+
          "\n\n\uD83D\uDC47 **Select a script below to continue:**"
        )
        .setColor(0x7c3aed);

      const menu=new ActionRowBuilder().addComponents(
        new StringSelectMenuBuilder().setCustomId("ticket_script_select")
          .setPlaceholder("\uD83D\uDCDC Select script to buy...")
          .addOptions(scripts.map(([id,s])=>({
            label: s.name,
            description: s.price+" coins | "+s.payment.join(", "),
            value: id,
            emoji: "\uD83D\uDCDC"
          })))
      );
      return interaction.reply({ embeds:[emb], components:[menu], ephemeral:true });
    }

    // Script selected → show payment method picker with embed
      ...
      const menu=new ActionRowBuilder().addComponents(
        new StringSelectMenuBuilder().setCustomId("ticket_pay_select__"+scriptId)
          ...
          .addOptions(script.payment.map(p=>{
            const payEmoji={ dana:"\uD83D\uDCB3", gopay:"\uD83D\uDCF2", robux:"\uD83C\uDFAE" };
            return {
    ...
    if (interaction.customId?.startsWith("ticket_pay_select__")) {
      ...
      try {
        ...
        const ticketRoleId=db.ticketCfg?.[guildId]?.ticketRoleId;
        const overwrites=[
          { id: interaction.guild.id, deny: [PermissionFlagsBits.ViewChannel] },
          { id: userId, allow: [PermissionFlagsBits.ViewChannel, PermissionFlagsBits.SendMessages, PermissionFlagsBits.ReadMessageHistory] },
        ];
        if (ticketRoleId) {
          overwrites.push({ id: ticketRoleId, allow: [PermissionFlagsBits.ViewChannel, PermissionFlagsBits.SendMessages, PermissionFlagsBits.ReadMessageHistory] });
        }
  ...
  const buildArgs = () => {
    switch(cmd) {
      ...
      case "ticketrole":   return [opts.getRole("role").id];
      case "alltickets":   return [opts.getString("status")||""];
